...
function canAccessThread(user, thread) {
  if (user.role === 'clinician') return true; // Attending doctors can consult with patients
  if (user.role === 'patient') return thread.toLowerCase() === user.name.toLowerCase(); // Patients: own private thread ONLY
  return false;
}

// GET /api/messages/:thread — strictly clinician and patient
router.get('/:thread', requireAuth, requireRole('clinician', 'patient'), async (req, res) => {
  ...
  if (!canAccessThread(req.user, thread)) {
    ...
    return res.status(403).json({ error: 'You can only view your own doctor-patient message thread.' });
  }
  const data = readAll();
...
// POST /api/messages { thread, to, text } — strictly clinician and patient
router.post('/', requireAuth, requireRole('clinician', 'patient'), async (req, res) => {
  ...
  if (!canAccessThread(req.user, thread)) {
    ...
    return res.status(403).json({ error: 'You can only message within your own doctor-patient thread.' });
  }

  const msg = { id: uuid(), thread, from: req.user.name, to: to || 'Doctor', text, ts: Date.now() };
  await update((d) => {
    if (!d.messages) d.messages = [];
    d.messages.push(msg);
  });
  await logAudit({ actor: req.user.name, role: req.user.role, type: 'access', action: `Sent a secure doctor-patient message in thread "${thread}"` });
