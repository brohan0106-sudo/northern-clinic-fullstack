...
async function seed() {
  ...
  // 6. Medical Records (Diagnoses, Notes, Vitals)
  ...
  // 16. Audit Logs & System Notifications
  const auditLog = [
  ...
  const mfaChallenges = {}; // tempToken -> { username, codeHash, expiresAt, attempts }

  // Backward compatibility alias for claims & insurance
  const insuranceData = insurancePolicies;
